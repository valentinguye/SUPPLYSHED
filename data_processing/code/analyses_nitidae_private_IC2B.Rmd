...
eu_certif0 <-
  ...
  summarise(.by = c(IS_DEST_EU, CERTIFICATION, NUM_COOP_SOURCED_DIRECT_VOLUME_BEQ, NUM_COOP_SOURCED_DIRECT_VAL),
  ...
  summarise(.by = c(IS_DEST_EU, CERTIFICATION),
            DIRECT_KTONNES_BEAN_EQ = 1e-6 * mean(COOP_SOURCED_DIRECT_VOLUME_BEQ)#,
            # CI_VOLUME_BEQ = qt(0.95/2+0.5, length(COOP_SOURCED_DIRECT_VOLUME_BEQ)-1) * 
            #   (sd(COOP_SOURCED_DIRECT_VOLUME_BEQ)/sqrt(length(COOP_SOURCED_DIRECT_VOLUME_BEQ))),
            
            # MEAN_PRICE_BEQ = mean(PRICE_BEQ),
            # CI_PRICE_BEQ = qt(0.95/2+0.5, length(PRICE_BEQ)-1) * 
            #   (sd(PRICE_BEQ)/sqrt(length(PRICE_BEQ)))
            ) %>% 
  arrange(IS_DEST_EU, CERTIFICATION) 
...
# Checks: 
if(eu_certif$SHARE_OF_DIRECT %>% sum() != 1){stop("problem")}
if(eu_certif$DIRECT_KTONNES_BEAN_EQ %>% sum() != direct_ktonnes_2022){ stop() }
  
direct_certified_ktonnes <-   
  eu_certif %>% filter(CERTIFIED) %>% pull(DIRECT_KTONNES_BEAN_EQ) %>% sum() 

direct_certified_noneu_ktonnes <- 
  eu_certif %>% filter(!IS_DEST_EU & CERTIFIED) %>% pull(DIRECT_KTONNES_BEAN_EQ) 
# corresponding pct 
pct_direct_being_certified_noneu <- 
  eu_certif %>% filter(!IS_DEST_EU & CERTIFIED) %>% pull(SHARE_OF_DIRECT) * 100
...
eu_certif %>% 
  select(YEAR, TOTAL_DIRECT_KTONNES_BEAN_EQ, CERTIFICATION, IS_DEST_EU, DIRECT_KTONNES_BEAN_EQ, SHARE_OF_DIRECT)
