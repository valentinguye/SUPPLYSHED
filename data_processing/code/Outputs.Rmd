...
## Normalize link probabilities - cell perspective
...
## Unconditional probabilistic LUs
...
fn_links_lus = function(links_data, link_proba_normed_var){
    ...
    ungroup() %>%
    mutate(
      LINK_AREA_HA =            cell_area_ha            * CELL_SHARE_COOP * !!as.symbol(link_proba_normed_var),
      LINK_COCOA_HA =           CELL_COCOA_HA           * CELL_SHARE_COOP * !!as.symbol(link_proba_normed_var),
      LINK_FOREST_HA =          CELL_FOREST_HA          * CELL_SHARE_COOP * !!as.symbol(link_proba_normed_var),
      LINK_DENSEFOREST_HA =     CELL_DENSEFOREST_HA     * CELL_SHARE_COOP * !!as.symbol(link_proba_normed_var),
      LINK_OTHERFORESTS_HA =    CELL_OTHERFORESTS_HA    * CELL_SHARE_COOP * !!as.symbol(link_proba_normed_var),
      LINK_ALL_CD_HA =          CELL_ALL_CD_HA          * CELL_SHARE_COOP * !!as.symbol(link_proba_normed_var),
      LINK_OLD_CD_HA =          CELL_OLD_CD_HA          * CELL_SHARE_COOP * !!as.symbol(link_proba_normed_var),
      LINK_REC_CD_HA =          CELL_REC_CD_HA          * CELL_SHARE_COOP * !!as.symbol(link_proba_normed_var),
      LINK_NDDEP_HA =           CELL_NDDEP_HA           * CELL_SHARE_COOP * !!as.symbol(link_proba_normed_var)
    ) 
}
...
links = 
  ...
  filter(any(LINK_PROBA_THRESH > 0)) %>%  
  mutate(
    LINK_AREA_NORMALISING_SUM = sum(LINK_AREA_HA),
    LINK_COCOA_NORMALISING_SUM = sum(LINK_COCOA_HA),
    LINK_FOREST_NORMALISING_SUM = sum(LINK_FOREST_HA),
    LINK_NDDEP_NORMALISING_SUM = sum(LINK_NDDEP_HA)
    # across(
    # .cols = starts_with("LINK_") & ends_with("_HA"), 
    # .fns = sum, 
    # .cols = paste0("{.col}","NORMALISING_SUM"))
  ) %>% 
  ungroup() %>% 
  mutate(
    LINK_AREA_HA_COOPNORMED = LINK_AREA_HA / LINK_AREA_NORMALISING_SUM, 
    LINK_COCOA_HA_COOPNORMED = LINK_COCOA_HA / LINK_COCOA_NORMALISING_SUM, 
    LINK_FOREST_HA_COOPNORMED = LINK_FOREST_HA / LINK_FOREST_NORMALISING_SUM, 
    LINK_NDDEP_HA_COOPNORMED = LINK_NDDEP_HA / LINK_NDDEP_NORMALISING_SUM 
    # across(
    # .cols = starts_with("LINK_") & ends_with("_HA"), 
    # .fns = {.col} / paste0("{.col}","NORMALISING_SUM"), 
    # .cols = )
  ) 
...
top_idcss_cells_sf = 
  ...
  # Represent discrete supply sheds by the cells with which coops have a positive unconditional proba 
  filter(LINK_PROBA_THRESH > 0) %>% 
  # Spatialize
  st_as_sf(coords = c("CELL_LONGITUDE", "CELL_LATITUDE"), crs = 4326) %>% 
...
# Split to plot in separate facets
# Top cocoa coops 
cells_top_cocoa_sf = 
  top_idcss_cells_sf %>% 
  inner_join(
...
top_cocoa_sf %>% arrange(desc(COOP_COCOA_HA)) %>% pull(COOP_COCOA_HA) %>% unique()
