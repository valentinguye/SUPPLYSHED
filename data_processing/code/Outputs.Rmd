...
land_use_labels_4map = 
  c("A. COCOA", "B. DENSE FOREST", "C. NO FOREST NOR COCOA")
...
```{r}
all_heuristics_dist_km = seq(5,150,5)
...
```{r}
# links_data = links
# link_proba_to_normalize = "LINK_PROBA"
...
# However, for heuristics, set it to zero, because this is the rule of heuristics. 
  if(link_proba_to_normalize == "LINK_PROBA"){
     links_data = 
       links_data %>% 
       mutate(
        LINK_PROBA_NORMED = case_when(
          NORMALISING_SUM > 0 ~ LINK_PROBA / NORMALISING_SUM, 
...
links = fn_normalize_link_proba(links_data = links, "LINK_PROBA")
...
fn_links_lus = function(links_data, link_proba_normed_var){
    links_data %>% 
    mutate(
      LINK_PROBA_UNCOND := CELL_SHARE_COOP * !!as.symbol(link_proba_normed_var),
      LINK_AREA_HA =            cell_area_ha         * LINK_PROBA_UNCOND,
      LINK_COCOA_HA =           CELL_COCOA_HA        * LINK_PROBA_UNCOND,
      LINK_FOREST_HA =          CELL_FOREST_HA       * LINK_PROBA_UNCOND,
      LINK_DENSEFOREST_HA =     CELL_DENSEFOREST_HA  * LINK_PROBA_UNCOND,
      LINK_OTHERFORESTS_HA =    CELL_OTHERFORESTS_HA * LINK_PROBA_UNCOND,
      LINK_ALL_CD_HA =          CELL_ALL_CD_HA       * LINK_PROBA_UNCOND,
      LINK_OLD_CD_HA =          CELL_OLD_CD_HA       * LINK_PROBA_UNCOND,
      LINK_REC_CD_HA =          CELL_REC_CD_HA       * LINK_PROBA_UNCOND,
      LINK_NDDEP_HA =           CELL_NDDEP_HA        * LINK_PROBA_UNCOND
    ) 
}

links = fn_links_lus(links, "LINK_PROBA")
```
...
fn_iccss_lus = function(links_data, link_proba_normed_var){
  linked_coops = 
    ...
    filter(any(LINK_PROBA > 0)) %>%
    ungroup() %>%
    summarise(.by = "LINK_POTENTIAL_COOP_ID", 
              COOP_UNWEIGHTED_AREA_HA = sum(cell_area_ha            * (CELL_SHARE_COOP > 0)),
...
links = 
  ...
  mutate(
    ...
    LINK_DENSEFOREST_COOPNORMALISING_SUM = sum(LINK_DENSEFOREST_HA, na.rm = TRUE),
  ...
  mutate(
    ...
    LINK_DENSEFOREST_HA_COOPNORMED = case_when(
      LINK_DENSEFOREST_COOPNORMALISING_SUM > 0 ~ LINK_DENSEFOREST_HA / LINK_DENSEFOREST_COOPNORMALISING_SUM,
      TRUE ~ 0
      ),
...
# Correction is for coops with very small predicted supply sheds, made of a single cell making alone more than 80%.  
links = 
  links %>% 
  group_by(LINK_POTENTIAL_COOP_ID) %>% 
  arrange(desc(LINK_AREA_HA_COOPNORMED)) %>% 
  # There are NAs in LINK_AREA_HA_COOPNORMED, that's links with out-of-AOA. cumsum does not accept na.rm argument but not a pb because arrange always puts NAs at the end.  
  mutate(LINK_AREA_HA_COOPNORMED_cumsum = cumsum(LINK_AREA_HA_COOPNORMED), 
         ...
         # correct it for small coops (see above) and NAs  
         LINK_AREA_IS_TOP_80PCT_COOP = case_when(
           !any(LINK_AREA_IS_TOP_80PCT_COOP) ~ TRUE, 
           TRUE ~ LINK_AREA_IS_TOP_80PCT_COOP),
         COOP_SHARE_CELLS_MAKING_80PCT_AREA = sum(LINK_AREA_IS_TOP_80PCT_COOP, na.rm = TRUE) / n(), 
         ) %>% 
  ungroup() %>% 
  group_by(LINK_POTENTIAL_COOP_ID) %>% 
  ...
  mutate(LINK_COCOA_HA_COOPNORMED_cumsum = cumsum(LINK_COCOA_HA_COOPNORMED), 
         ...
         # correct it for small coops
         LINK_COCOA_IS_TOP_80PCT_COOP = case_when(
           !any(LINK_COCOA_IS_TOP_80PCT_COOP) ~ TRUE, 
           TRUE ~ LINK_COCOA_IS_TOP_80PCT_COOP), 
         # calculate the share of cells making these 80%
         COOP_SHARE_CELLS_MAKING_80PCT_COCOA = sum(LINK_COCOA_IS_TOP_80PCT_COOP, na.rm = TRUE) / n()
         ) %>% 
  ungroup() %>% 
  group_by(LINK_POTENTIAL_COOP_ID) %>% 
  ...
  mutate(LINK_FOREST_HA_COOPNORMED_cumsum = cumsum(LINK_FOREST_HA_COOPNORMED), 
         ...
         # correct it for small coops
         LINK_FOREST_IS_TOP_80PCT_COOP = case_when(
           !any(LINK_FOREST_IS_TOP_80PCT_COOP) ~ TRUE, 
           TRUE ~ LINK_FOREST_IS_TOP_80PCT_COOP), 
         # calculate the share of cells making these 80%
         COOP_SHARE_CELLS_MAKING_80PCT_FOREST = sum(LINK_FOREST_IS_TOP_80PCT_COOP, na.rm = TRUE) / n()
         ) %>% 
  ungroup() %>% 
  group_by(LINK_POTENTIAL_COOP_ID) %>% 
  arrange(desc(LINK_DENSEFOREST_HA_COOPNORMED)) %>% 
  mutate(LINK_DENSEFOREST_HA_COOPNORMED_cumsum = cumsum(LINK_DENSEFOREST_HA_COOPNORMED), 
         LINK_DENSEFOREST_IS_TOP_80PCT_COOP = (LINK_DENSEFOREST_HA_COOPNORMED_cumsum <= 0.8), 
         # correct it for small coops
         LINK_DENSEFOREST_IS_TOP_80PCT_COOP = case_when(
           !any(LINK_DENSEFOREST_IS_TOP_80PCT_COOP) ~ TRUE, 
           TRUE ~ LINK_DENSEFOREST_IS_TOP_80PCT_COOP), 
         # calculate the share of cells making these 80%
         COOP_SHARE_CELLS_MAKING_80PCT_DENSEFOREST = sum(LINK_DENSEFOREST_IS_TOP_80PCT_COOP, na.rm = TRUE) / n()
         ) %>% 
  ungroup() %>% 
  group_by(LINK_POTENTIAL_COOP_ID) %>% 
  ...
  mutate(LINK_NDDEP_HA_COOPNORMED_cumsum = cumsum(LINK_NDDEP_HA_COOPNORMED), 
         ...
         # correct it for small coops
         LINK_NDDEP_IS_TOP_80PCT_COOP = case_when(
           !any(LINK_NDDEP_IS_TOP_80PCT_COOP) ~ TRUE, 
           TRUE ~ LINK_NDDEP_IS_TOP_80PCT_COOP), 
         # calculate the share of cells making these 80%
         COOP_SHARE_CELLS_MAKING_80PCT_NDDEP = sum(LINK_NDDEP_IS_TOP_80PCT_COOP, na.rm = TRUE) / n()
         ) %>% 
  ungroup() 

links %>% filter(LINK_POTENTIAL_COOP_ID == "COOP-1000") %>% arrange(desc(LINK_COCOA_HA_COOPNORMED)) %>% 
 select(LINK_POTENTIAL_COOP_ID, starts_with("LINK_COCOA"), starts_with("COOP_SHARE_CELLS_")) %>% View()
...
```{r}
iccss = 
  fn_iccss_lus(links_data = links, link_proba_normed_var = "LINK_PROBA_NORMED")

iccss$COOP_AREA_HA %>% summary()
quantile(iccss$COOP_AREA_HA, seq(0,1,0.1))
quantile(iccss$COOP_AREA_HA, seq(0.9,1,0.01))

links %>% 
  summarise(.by = "LINK_POTENTIAL_COOP_ID", 
            COOP_SHARE_CELLS_MAKING_80PCT_AREA = mean(COOP_SHARE_CELLS_MAKING_80PCT_AREA),
            COOP_SHARE_CELLS_MAKING_80PCT_COCOA = mean(COOP_SHARE_CELLS_MAKING_80PCT_COCOA),
            COOP_SHARE_CELLS_MAKING_80PCT_FOREST = mean(COOP_SHARE_CELLS_MAKING_80PCT_FOREST),
            COOP_SHARE_CELLS_MAKING_80PCT_NDDEP = mean(COOP_SHARE_CELLS_MAKING_80PCT_NDDEP)) %>% summary()


# 
# How large is the unweighted area compared to the weighted area? 
iccss$COOP_UNWEIGHTED_AREA_HA %>% summary()
quantile(iccss$COOP_UNWEIGHTED_AREA_HA, seq(0,1,0.1))
quantile(iccss$COOP_UNWEIGHTED_AREA_HA, seq(0.9,1,0.01))
...
Prepare
```{r}
# This excludes coops with no positive predicted link (none now, since we don't threshold proba.)
linked_coops = 
  fn_iccss_lus(links_data = links, link_proba_normed_var = "LINK_PROBA_NORMED")
...
# actually one coop is excluded from linked_coops, possibly because of the AOA. 
coop22_iccss_sf$COOP_AREA_HA %>% summary()
...
top_total_cells_sf$LINK_PROBA_UNCOND %>% summary()
...
total_topcoops_plot = 
  ...
  geom_sf(data = top_total_cells_sf, aes(fill = LINK_PROBA_UNCOND, col = LINK_PROBA_UNCOND)) + 
  scale_fill_viridis_c(na.value = "transparent", name = "Supply probability",
                       labels=unit_format(unit = "", sep = ""), 
                       option = "mako", direction = -1) +
 scale_color_viridis_c(na.value = "transparent", name = "Supply probability",
                       labels=unit_format(unit = "", sep = ""), 
                       option = "mako", direction = -1) +
  geom_sf(data = departements, fill = "transparent", col = "black") +
...
bot_total_cells_sf = 
  links %>% 
  mutate(COOP_ID = LINK_POTENTIAL_COOP_ID) %>% 
  # DO NOT filter to the cells making the top 80% of the LU in each ICCSS, because this is useless with current formula for 80PCT 
  inner_join(
...
bot_total_cells_sf$LINK_PROBA_UNCOND %>% summary()

total_botcoops_plot = 
  ggplot() +
  # Plot the unconditional probability rather than the total area (which is equivalent in spatial relativity, but is more intuitive to see at this point). 
  geom_sf(data = bot_total_cells_sf, aes(fill = LINK_PROBA_UNCOND, col = LINK_PROBA_UNCOND)) + 
  scale_fill_viridis_c(na.value = "transparent", name = "Supply probability",
                       labels=unit_format(unit = "", sep = ""), 
                       option = "mako", direction = -1) +
 scale_color_viridis_c(na.value = "transparent", name = "Supply probability",
...
### Multiple buying stations
```{r}
linked_coops$COOP_AREA_HA %>% summary()
quantile(linked_coops$COOP_AREA_HA, seq(0,1,0.1))
xmultibs = 5

multibstotal_coops_sf = 
  coop22_iccss_sf %>% 
  arrange(desc(COOP_N_KNOWN_BS)) %>% # THE ONLY THING CHANGING COMPARED TO ABOVE
  head(xmultibs)

multibs_total_cells_sf = 
  links %>% 
  mutate(COOP_ID = LINK_POTENTIAL_COOP_ID) %>% 
  # Represent discrete supply sheds by the cells making the top 80% of the LU in each ICCSS 
  filter(LINK_AREA_IS_TOP_80PCT_COOP > 0) %>% 
  inner_join(
    multibstotal_coops_sf %>% st_drop_geometry(), 
    by = "COOP_ID"
  ) %>% 
  # Spatialize
  st_as_sf(coords = c("CELL_LONGITUDE", "CELL_LATITUDE"), crs = 4326) %>% 
...
# make square shape of cells
st_geometry(multibs_total_cells_sf) <- 
  sapply(st_geometry(multibs_total_cells_sf), FUN = function(x){st_as_sfc(st_bbox(x))}) %>% st_sfc(crs = civ_crs)

# Values at multiple bs coops
multibs_total_cells_sf %>% arrange(desc(COOP_AREA_HA)) %>% pull(COOP_AREA_HA) %>% unique()
# Values in cells
multibs_total_cells_sf$LINK_AREA_HA %>% summary()
multibs_total_cells_sf$LINK_PROBA_UNCOND %>% summary()

# Retrive the points of the BS 
multibstotal_bs_sf = 
  coopbs22_sf %>%  
  filter(COOP_ID %in% multibstotal_coops_sf$COOP_ID)

total_multibscoops_plot = 
  ggplot() +
  # Plot the unconditional probability rather than the total area (which is equivalent in spatial relativity, but is more intuitive to see at this point). 
  geom_sf(data = multibs_total_cells_sf, aes(fill = LINK_PROBA_UNCOND, col = LINK_PROBA_UNCOND)) + 
  scale_fill_viridis_c(na.value = "transparent", name = "Supply probability",
                       labels=unit_format(unit = "", sep = ""), 
                       option = "mako", direction = -1) +
 scale_color_viridis_c(na.value = "transparent", name = "Supply probability",
  ...
  geom_sf(data = departements, fill = "transparent", col = "black") +
  geom_sf(data = coopbs22_sf, col = "black", size = 0.3) +
  geom_sf(data = multibstotal_bs_sf, col = "purple", size = 0.8) +
  theme_bw() + 
  guides(colour = guide_legend(override.aes = list(size=10))) +
  scale_x_continuous(breaks = xlabs, labels = paste0(xlabs,'°W')) +
  scale_y_continuous(breaks = ylabs, labels = paste0(ylabs,'°N')) +
...
ggsave(
  plot = total_multibscoops_plot,
  filename = paste0("total_multibscoops_plot.png"),
  path = here("outputs", "individual_sheds"),
  width = 25,
  height = 20,
  units = "cm")
...
# quantile(linked_coops$COOP_AREA_HA, seq(0,1,0.1))
# xclustered = 5
# # THE ONLY THING CHANGING COMPARED TO ABOVE - ce n'est pas au point. Standby for now. 
# clusteredtotal_coops_sf = 
#   rbind(
#     coop22_iccss_sf %>% 
#       inner_join(
#         coop22_sf %>% filter(DISTRICT_NAME == "ABIDJAN") %>% st_drop_geometry(), 
#         by = "COOP_ID"
#       ) %>% 
#       head(3),
#     coop22_iccss_sf %>% 
#       inner_join(
#         coop22_sf %>% filter(DISTRICT_NAME == "SAN-PEDRO") %>% st_drop_geometry(), 
#         by = "COOP_ID"
#       ) %>% 
#       head(3)
#   )
# 
# clustered_total_cells_sf = 
#   links %>% 
#   mutate(COOP_ID = LINK_POTENTIAL_COOP_ID) %>% 
#   # Represent discrete supply sheds by the cells making the top 80% of the LU in each ICCSS 
#   filter(LINK_AREA_IS_TOP_80PCT_COOP > 0) %>%   
#   inner_join(
#     clusteredtotal_coops_sf %>% st_drop_geometry(), 
#     by = "COOP_ID"
#   ) %>% 
#   # Spatialize
#   st_as_sf(coords = c("CELL_LONGITUDE", "CELL_LATITUDE"), crs = 4326) %>% 
...
# 
# # make square shape of cells
# st_geometry(clustered_total_cells_sf) <- 
#   sapply(st_geometry(clustered_total_cells_sf), FUN = function(x){st_as_sfc(st_bbox(x))}) %>% st_sfc(crs = civ_crs)
# 
# # Values at multiple bs coops
# clustered_total_cells_sf %>% arrange(desc(COOP_AREA_HA)) %>% pull(COOP_AREA_HA) %>% unique()
# # Values in cells
# clustered_total_cells_sf$LINK_AREA_HA %>% summary()
# clustered_total_cells_sf$LINK_PROBA_UNCOND %>% summary()
# 
# total_clusteredcoops_plot = 
#   ggplot() +
#   # Plot the unconditional probability rather than the total area (which is equivalent in spatial relativity, but is more intuitive to see at this point). 
#   geom_sf(data = clustered_total_cells_sf, aes(fill = LINK_PROBA_UNCOND, col = LINK_PROBA_UNCOND)) + 
#   scale_fill_viridis_c(na.value = "transparent", name = "Supply probability",
#                        labels=unit_format(unit = "", sep = ""), 
...
#   geom_sf(data = departements, fill = "transparent", col = "black") +
#   geom_sf(data = coopbs22_sf, col = "black", size = 0.3) +
#   geom_sf(data = clusteredtotal_coops_sf, col = "red", size = 0.8) +
#   theme_bw() + 
#   guides(colour = guide_legend(override.aes = list(size=10))) +
#   scale_x_continuous(breaks = xlabs, labels = paste0(xlabs,'°W')) +
#   scale_y_continuous(breaks = ylabs, labels = paste0(ylabs,'°N')) +
...
# 
# ggsave(
#   plot = total_clusteredcoops_plot,
#   filename = paste0("total_clusteredcoops_plot.png"),
#   path = here("outputs", "individual_sheds"),
#   width = 25,
#   height = 20,
#   units = "cm")
...
multipanel_case_studies = 
  ggarrange(
    ...
    ncol = 3, nrow = 1, 
    common.legend = FALSE, 
    labels = c(paste0("A. ",xtop, " WIDEST"), paste0("B. ",xbot," SMALLEST"), 
               "C. MULTIPLE BUYING STATIONS"), 
    hjust = c(-0.5, -0.5, -0.2), 
...
# Let's keep that title for the Ggdoc
# CASE STUDIES OF INDIVIDUAL CONTINUOUS COOPERATIVE SUPPLY SHED \n

multipanel_case_studies

ggsave(
  plot = multipanel_case_studies,
  filename = paste0("multipanel_case_studies.png"),
  path = here("outputs", "individual_sheds"),
  width = 75,
  height = 50,
  units = "cm")
...
fn_IoU_atlink = function(heur_link_proba_var){
  linked_coops =
    ...
    # remove cells outside AOA).   
    filter(!is.na(CELL_SHARE_COOP)) %>% 
    # # remove coops without any potential link predicted with positive proba. - this makes no difference now that we do not threshold LINK_PROBA as a class probability. 
    # group_by(LINK_POTENTIAL_COOP_ID) %>% 
    # filter(any(LINK_PROBA > 0)) %>%
    # ungroup() %>% 
    # filter(LINK_AREA_IS_TOP_80PCT_COOP | !!as.symbol(heur_link_proba_var) > 0)
    summarise(.by = "LINK_POTENTIAL_COOP_ID", 
      # COOP_IOU_notnorm := sum(LINK_PROBA > 0 & !!as.symbol(heur_link_proba_var) > 0) /
      #                     sum(LINK_PROBA > 0 | !!as.symbol(heur_link_proba_var) > 0),
      
      COOP_IOU := sum(LINK_AREA_IS_TOP_80PCT_COOP & (!!as.symbol(heur_link_proba_var) > 0)) /
                  sum(LINK_AREA_IS_TOP_80PCT_COOP | (!!as.symbol(heur_link_proba_var) > 0)),
      
      COOP_IOU_COCOA := sum(LINK_COCOA_IS_TOP_80PCT_COOP & (!!as.symbol(heur_link_proba_var)*CELL_COCOA_HA > 0)) /
                        sum(LINK_COCOA_IS_TOP_80PCT_COOP | (!!as.symbol(heur_link_proba_var)*CELL_COCOA_HA > 0))
      
      # COOP_N_CELLS_MODEL = sum(LINK_PROBA > 0),
      # COOP_N_CELLS_MODEL_NORMED = sum(LINK_PROBA_NORMED > 0),
      # COOP_N_CELLS_MODEL_UNCOND_NORMED = sum(LINK_AREA_IS_TOP_80PCT_COOP)
    ) 
  return(linked_coops$COOP_IOU)
}

# can have cells in neither model or heuristic supply shed, leading to sum(LINK_AREA_IS_TOP_80PCT_COOP | (!!as.symbol(heur_link_proba_var) > 0)) being 0 and the IOU being NaN.  
...
mean_data <- data.frame(
  name = names(IoU_list) %>% as.numeric(),# factor(levels=1:length(names(IoU_list))),
...
heuristic_curve = 
  ggplot(mean_data, aes(x = name, y = mean_value, group = 1)) +
    ...
    labs(title = "", # Mean and 95% Confidence Interval of Intersection over Union
         x = "Heuristic radius (km)",
         y = "IoU") +
...
ggsave(
  plot = heuristic_curve,
  filename = paste0("heuristic_curve.png"),
  path = here("outputs", "individual_sheds"),
  width = 25,
  height = 20,
  units = "cm")
...
# # links_topredict$LINK_PROBA %>% summary()
# quantile(links$LINK_PROBA, seq(0, 1, 0.1), na.rm = T)
# quantile(links$LINK_PROBA, seq(0.9, 1, 0.01), na.rm = T)
# liklink = links %>% filter(LINK_PROBA > 0) 
...
## 5.1.4. Factors of heterogeneity
...
# ggsave(
#   plot = out_tbl,
#   filename = paste0("ACCSS_allLU_table.png"),
...
Use palettes with blue: BlueGold, DkViolet2, BlueYl (similar), BlueOr, DkViolet2
...
map =
  ...
  geom_sf(data = coopbs22_sf, col = "black", size = 0.3) +
  bi_scale_fill(pal = "DkViolet2", dim = 3, na.value = "transparent") +
  bi_scale_color(pal = "DkViolet2", dim = 3, na.value = "transparent") +
  # labs(
...
BlueYl
```{r}
cells_toplot$CELL_DENSEFOREST_SHARE %>% summary()

# This makes the bivariate bins
cells_toplot = 
  cells_toplot %>% 
  bi_class(CELL_DENSEFOREST_SHARE,
            `Cooperative outlet (%)`, 
            dim = 3, 
            style = "quantile", 
                  dig_lab = 9)
...
breaks = 
  cells_toplot %>% 
  bi_class_breaks(CELL_DENSEFOREST_SHARE,
                  `Cooperative outlet (%)`, 
                  dim = 3, 
                  style = "quantile", 
                  dig_lab = 4, 
...
map =
  ...
  geom_sf(data = departements, fill = "transparent", col = "darkgrey") +
  geom_sf(data = coopbs22_sf, col = "black", size = 0.3) +
  bi_scale_fill(pal = "BlueYl", flip_axes = FALSE, dim = 3, na.value = "transparent") +
  bi_scale_color(pal = "BlueYl", flip_axes = FALSE, dim = 3, na.value = "transparent") +
  # labs(
...
legend <- bi_legend(pal = "BlueYl",
                    ...
                    ylab = "Cooperative outlet (%)",
                    size = 10, 
                    breaks = breaks, flip_axes = FALSE, # rotate_pal = T,
                    arrows = FALSE)
...
map =
  ...
  geom_sf(data = departements, fill = "transparent", col = "darkgrey") +
  geom_sf(data = coopbs22_sf, col = "black", size = 0.3) +
  bi_scale_fill(pal = "BlueOr", flip_axes = FALSE, dim = 3, na.value = "transparent") +
  bi_scale_color(pal = "BlueOr", flip_axes = FALSE, dim = 3, na.value = "transparent") +
  # labs(
...
legend <- bi_legend(pal = "BlueOr", flip_axes = FALSE,
                    dim = 3,
                    xlab = "Other land (%)",
                    ylab = "Cooperative outlet (%)",
                    size = 10, 
...
cell_nlinks = 
  ...
  summarise(.by = "CELL_ID",
            # CELL_N_LINKED_COOPS = sum(LINK_PROBA_NORMED_CELLMAX > 0), 
            CELL_N_LINKED_COOPS_ALT = sum(LINK_PROBA_NORMED > 0)
            )
...
cells$CELL_N_LINKED_COOPS %>% summary()
...
quantile(linked_coops$COOP_COCOA_HA, seq(0.9,1,0.01))

linked_coops$COOP_FOREST_HA %>% summary()
quantile(linked_coops$COOP_FOREST_HA, seq(0.9,1,0.01))

linked_coops$COOP_NDDEP_HA %>% summary()
quantile(linked_coops$COOP_NDDEP_HA, seq(0.9,1,0.01))
...
topcocoa_coops_sf$COOP_COCOA_HA %>% summary()
...
topforest_coops_sf$COOP_DENSEFOREST_HA %>% summary()
...
topnddep_coops_sf$COOP_NDDEP_HA %>% summary()
...
cells_top_DENSEFOREST_sf = 
  top_idcss_cells_sf %>% 
  # Represent discrete supply sheds by the cells making the top 80% of the LU in each ICCSS 
  filter(LINK_DENSEFOREST_IS_TOP_80PCT_COOP > 0) %>% 
  inner_join(
    topforest_coops_sf %>% st_drop_geometry(), 
    by = "COOP_ID")
...
cocoa_topcoops_plot = 
  ...
  scale_fill_viridis_c(na.value = "transparent", name = "Expected area",
                       labels=unit_format(unit = "ha", sep = ""), 
                       option = "rocket") +
 scale_color_viridis_c(na.value = "transparent", name = "",
                     labels=unit_format(unit = "ha", sep = "Expected area"), 
                     option = "rocket") +
  geom_sf(data = departements, fill = "transparent", col = "black") +
  geom_sf(data = coopbs22_sf, col = "black", size = 0.3) +
...
forest_topcoops_plot = 
  ggplot() +
  geom_sf(data = cells_top_DENSEFOREST_sf, aes(fill = LINK_DENSEFOREST_HA, col = LINK_DENSEFOREST_HA)) + 
  scale_fill_viridis_c(na.value = "transparent", name = "Expected area",
                       labels=unit_format(unit = "ha", sep = ""), 
                       option = "viridis") +
 scale_color_viridis_c(na.value = "transparent", name = "Expected area",
                     labels=unit_format(unit = "ha", sep = ""), 
                     option = "viridis") +
  geom_sf(data = departements, fill = "transparent", col = "black") +
  geom_sf(data = coopbs22_sf, col = "black", size = 0.3) +
...
multipanel_hotspots = 
  ggarrange(
    ...
    annotate_figure(top = text_grob(paste0("TOP ",xtop," COOPERATIVE SUPPLY SHEDS WITH MOST...  \n"), face = "bold", size = 18))
...
coop22_sf = 
  ...
  left_join(linked_coops, 
            by = join_by(COOP_ID == LINK_POTENTIAL_COOP_ID)) %>% 
  arrange(desc(COOP_COCOA_HA)) %>% 
  filter(!is.na(COOP_COCOA_RANK)) # remove this single coop not in linked_coops. 
...
coop22_sf = 
  ...
  left_join(linked_coops, 
            by = join_by(COOP_ID == LINK_POTENTIAL_COOP_ID)) %>% 
  arrange(desc(COOP_DENSEFOREST_HA)) %>% 
  filter(!is.na(COOP_DENSEFOREST_RANK)) # remove this single coop not in linked_coops. 
...
quantile(linked_coops$COOP_NDDEP_HA, seq(0,1,0.1))
quantile(linked_coops$COOP_PROP_OF_ACCSS_NDDEP, seq(0,1,0.1))

# Calculate the cumulative sum of COOP_NDDEP_HA
linked_coops <- linked_coops %>%
  arrange(desc(COOP_NDDEP_HA))
linked_coops <- linked_coops %>%
  mutate(COOP_NDDEP_HA_cumulative_sum = cumsum(COOP_NDDEP_HA))

# Calculate the total sum 
total_sum <- sum(linked_coops$COOP_NDDEP_HA)

# Calculate the cumulative percentage of COOP_NDDEP_HA
linked_coops <- linked_coops %>%
  mutate(
    COOP_NDDEP_RANK_cumulative_percentage        = 100*COOP_NDDEP_HA_cumulative_sum / total_sum, 
    COOP_NDDEP_RANK = case_when(
       COOP_NDDEP_RANK_cumulative_percentage < 20 ~ "Top 20%",
       COOP_NDDEP_RANK_cumulative_percentage >= 20 & COOP_NDDEP_RANK_cumulative_percentage < 80 ~ "20-80%", 
       COOP_NDDEP_RANK_cumulative_percentage >= 80 ~ "Bottom 20%"
       )
    )
    
linked_coops$COOP_NDDEP_RANK %>% table()
...
# model
decile_counts <- sapply(deciles, function(d) {
  sum(linked_coops$COOP_NDDEP_RANK_cumulative_percentage <= d)
})
decile_results <- data.frame(
  Decile = deciles,
  Count = decile_counts
...
coop22_sf = 
  ...
  left_join(linked_coops, 
            by = join_by(COOP_ID == LINK_POTENTIAL_COOP_ID)) %>% 
  arrange(desc(COOP_NDDEP_HA)) %>% 
  filter(!is.na(COOP_NDDEP_RANK)) # remove this single coop not in linked_coops. 
...
coop22_sf = 
  coop22_sf %>% 
  mutate(
    COOP_NDDEP_RANK = factor(COOP_NDDEP_RANK, levels = c("Bottom 20%","20-80%","Top 20%")))

map_hotspot_coops_dense_nddep = 
 ggplot(coop22_sf) +
  geom_sf(mapping = aes(size = COOP_NDDEP_RANK, fill = COOP_NDDEP_HA),
         shape = 21) + #, col = COOP_NDDEP_HA
  ...
  scale_y_continuous(breaks = ylabs, labels = paste0(ylabs,'°N')) +
  # labs(
  #   title = "ICCSS hotspots of no-direct-deforestation potential",
  #   subtitle = ""
  # ) + 
   guides(size=guide_legend(title="Relative to all cooperatives"))

ggsave(
  plot = map_hotspot_coops_dense_nddep,
  filename = paste0("map_hotspot_coops_nddep.png"),
  path = here("outputs", "individual_sheds"),
  width = 25,
  height = 20,
  units = "cm")
...
multipanel_case_studies = 
  ggarrange(
    ...
    ncol = 3, nrow = 1, 
    common.legend = FALSE, 
    labels = c("A. COCOA PLANTED IN 2020", "B. REMAINING DENSE FOREST", 
               "C. NO-DIRECT-DEFORESTATION POTENTIAL"), 
    hjust = c(-0.5, -0.5, -0.2), 
    font.label = list(size = 14, color = "black", face = "bold", family = NULL)
    # align = "h"
  ) %>% 
    annotate_figure(top = text_grob("ICCSS HOTSPOTS \n", face = "bold", size = 18))
...
links = 
  ...
  group_by(CELL_ID) %>% 
  mutate(
    LINK_PROBA_NORMED_CELLMAX = max(LINK_PROBA_NORMED), 
  ) %>% 
  ungroup() %>% 
  mutate(LINK_IS_MOST_LIKELY = case_when(
    LINK_PROBA_NORMED_CELLMAX == 0 | # where all probas are null
      # or where their maximum is strictly higher than the link's proba, the link is NOT most likely
    LINK_PROBA_NORMED_CELLMAX > LINK_PROBA_NORMED ~ FALSE, 
    is.na(LINK_PROBA_NORMED_CELLMAX) ~ NA,
    TRUE ~ TRUE
    )
  ) %>% 
  group_by(CELL_ID) %>% 
...
# links %>% 
#   group_by(CELL_ID) %>% 
#   mutate(CELL_WEIGHTS_SUM = sum(LINK_PROBA_NORMED)) %>% 
#   ungroup() %>% 
...
links_cellproba %>% 
  select(CELL_ID, LINK_PROBA_NORMED, LINK_POTENTIAL_COOP_ID, COOP_CERTIFIED, CELL_PROBA_COOP_CERTIFIED) %>% 
  arrange(CELL_ID, desc(LINK_PROBA_NORMED)) %>% 
  View()
...
cell_exp =
  ...
  summarise(.by = CELL_ID, 
            across(.cols = all_of(vars_to_distribute), 
                   .fns = ~sum(.x*LINK_PROBA_NORMED), # THIS IS THE DIFFERENCE WITH PREVIOUS CODE BLOCK
                   .names = paste0("PROBA_", "{.col}")), 
            CELL_LONGITUDE = unique(CELL_LONGITUDE),
            CELL_LATITUDE = unique(CELL_LATITUDE)
            )
...
links = 
  ...
  mutate(
    ...
    CELL_INFLUENCE_SHED_ONLY = CELL_SHARE_COOP == 0 & LINK_PROBA_NORMED_CELLMAX > 0, 
    ...
    CELL_PRODUCTIVE_INFLUENCE_SHED_ONLY = CELL_SHARE_COOP == 0 & LINK_PROBA_NORMED_CELLMAX > 0 & CELL_PRODUCTION_STATUS == "PRODUCTIVE",
    ...
    CELL_PROSPECTIVE_INFLUENCE_SHED_ONLY = CELL_SHARE_COOP == 0 & LINK_PROBA_NORMED_CELLMAX > 0 & CELL_PRODUCTION_STATUS == "PROSPECTIVE", 
