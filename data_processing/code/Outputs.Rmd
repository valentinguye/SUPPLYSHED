...
# Calculate the IoU at link level
...
fn_iccss_total = function(links_data, link_proba_var){
  linked_coops = 
    links_data %>% 
    summarise(.by = "LINK_POTENTIAL_COOP_ID", 
              COOP_AREA_HA := sum(cell_area_ha * CELL_SHARE_COOP * !!as.symbol(link_proba_var))
    )
}
...
links = 
  ...
  mutate(
    ...
    # However, for heuristics, set it to zero, because this is the rule of heuristics. 
    # Ok but the thing is that we will never know for sure, even with first stage model. Precisely, the first stage model 
    # is not super performant, so we might want to leverage information from the links data to learn about these places that sell to no coop. 
