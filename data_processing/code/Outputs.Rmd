...
fn_aggregate_lu_relative = function(out_or_in_share){
  ...
  summarise(.by = CELL_IS_IN_AOA, 
            ...
            FOREST         = 100*round(sum({{out_or_in_share}}*(CELL_DENSEFOREST_HA+CELL_OTHERFORESTS_HA)) / sum((CELL_DENSEFOREST_HA+CELL_OTHERFORESTS_HA)), 1),
...
cells_toplot$`Cooperative outlet (%)` = 
  cut(cells_toplot$CELL_SHARE_COOP, 
      ...
      include.lowest = TRUE,
      breaks = c(0,15,45,max_share_coop), 
      labels = bin_names
      )
...
map =
  ...
  geom_sf(data = coopbs22_sf, col = "black", size = 3) +
...
BlueOr
```{r}
cells_toplot$CELL_FOREST_HA %>% summary()

# This makes the bivariate bins
cells_toplot = 
  cells_toplot %>% 
  bi_class(CELL_FOREST_HA,
            `Cooperative outlet (%)`, 
            dim = 3, 
            style = "quantile", 
                  dig_lab = 9)
...
breaks = 
  cells_toplot %>% 
  bi_class_breaks(CELL_FOREST_HA,
                  `Cooperative outlet (%)`, 
                  dim = 3, 
                  style = "quantile", 
                  dig_lab = 4) 
...
legend <- bi_legend(pal = "BlueOr",
                    dim = 3,
                    xlab = "Extent of forested land (ha)",
                    ylab = "Cooperative outlet (%)",
                    size = 6, 
                    breaks = breaks)
...
DkViolet2
```{r}
cells_toplot$CELL_NODEF_EXPANSION_HA %>% summary()

# This makes the bivariate bins
cells_toplot = 
  cells_toplot %>% 
  bi_class(CELL_NODEF_EXPANSION_HA,
            `Cooperative outlet (%)`, 
            dim = 3, 
            style = "quantile", 
                  dig_lab = 9)
...
breaks = 
  cells_toplot %>% 
  bi_class_breaks(CELL_NODEF_EXPANSION_HA,
                  `Cooperative outlet (%)`, 
                  dim = 3, 
                  style = "quantile", 
                  dig_lab = 4) 
...
legend <- bi_legend(pal = "DkViolet2",
                    dim = 3,
                    xlab = "Extent of available land (ha)",
                    ylab = "Cooperative outlet (%)",
                    size = 6, 
                    breaks = breaks)
