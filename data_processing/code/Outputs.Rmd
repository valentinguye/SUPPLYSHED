...
links = readRDS(here("temp_data", "model", "links_topredict_nopostprocessing.Rdata"))
...
cells = 
  ...
  mutate(
    ...
    CELL_NODEF_EXPANSION_SHARE = CELL_NODEF_EXPANSION_HA / cell_area_ha,
...
links$LINK_PROBA_THRESH_NORMED %>% summary() 
links$LINK_PROBA_HCONTI_NORMED %>% summary() 
links$LINK_PROBA_H60KM_NORMED %>% summary() 
# links %>% 
#   filter(round(CELL_UNCOND_NORMSUM_THRESH,10) != round(CELL_SHARE_COOP * CELL_NORMSUM_THRESH, 10)) %>% nrow()

stopifnot(
links %>% filter(is.na(LINK_PROBA_THRESH_NORMED)) %>% nrow() == 0 
)
stopifnot(
links %>% filter(is.na(LINK_PROBA_H60KM_NORMED)) %>% nrow() == 0 
)
...
stopifnot(
  ...
  # + length(unique(pull(filter(links, CELL_NORMSUM_THRESH == 0), CELL_ID))) # that was for when we (wrongly) set conditional probas to 0 where normsum = 0
  == 
    length(unique(links$CELL_ID)) |
    
  links$LINK_PROBA_H60KM_NORMED %>% sum(na.rm = T) %>% round(5) +
    length(unique(links$CELL_ID)) |

  links$LINK_PROBA_HDISTR_NORMED %>% sum(na.rm = T) %>% round(5) +
    length(unique(links$CELL_ID))
)
...
cells_toplot = 
  ...
  mutate(
   # Work on PERCENTAGE, not share, from here, for display. 
    ...
    CELL_NODEF_EXPANSION_SHARE = 100*CELL_NODEF_EXPANSION_SHARE, 
    # Round hectares
...
Helper functions
```{r}
# Make heuristic probability variables 
fn_make_heuristic_proba = function(links_data, heuristic_dist_km){
...
# Normalize a link probability variable
...
links_data = 
  ...
  group_by(CELL_ID) %>% 
  mutate(
    NORMALISING_SUM = sum(link_proba_to_normalize), 
  ) %>% 
  ungroup() 
...
stopifnot(
links %>% filter(is.na(!!as.symbol(paste0(link_proba_to_normalize,"_NORMED")))) %>% nrow() == 0 
)

stopifnot(
  links %>% select(link_proba_to_normalize) %>% sum(na.rm = T) %>% round(5)  == length(unique(links$CELL_ID)) 
  # + length(unique(pull(filter(links, CELL_NORMSUM_THRESH == 0), CELL_ID))) # that was for when we (wrongly) set conditional probas to 0 where normsum = 0
)
  
} 

# Calculate the total area of the ICCSS for each coop, for a given link probability variable (which will be the modeled one, or heuristic ones). 
fn_iccss_total = function(links_data, link_proba_var){
  links_data
}
...
## 5.1.2. Heuristics
We need to determine, for every link, whether it exists under a given heuristic distance or not. 
Then, formulate the IoU for a given probability threshold and heuristic distance. 
...
links$LINK_PROBA_H10KM %>% summary()
links$LINK_PROBA_H30KM %>% summary()
links$LINK_PROBA_H60KM %>% summary()
links$LINK_PROBA_HDISTR %>% summary()
links$LINK_PROBA_HCONTI %>% summary()
...
fn_aggregate_lu = function(out_or_in_share){
  ...
  summarise(.by = CELL_IS_IN_AOA, # this is to summarise over all obs, because non-AOA cells have been remove in previous script already.
            ...
            `OTHER AGRICULTURAL LAND` = round(sum({{out_or_in_share}}*CELL_NODEF_EXPANSION_HA) / 1000, 1), 
...
fn_aggregate_lu_relative = function(out_or_in_share){
  ...
  summarise(.by = CELL_IS_IN_AOA, 
            ...
            `OTHER AGRICULTURAL LAND` = 100*round(sum({{out_or_in_share}}*CELL_NODEF_EXPANSION_HA) / sum(CELL_NODEF_EXPANSION_HA), 2),
...
DkViolet2
```{r}
cells_toplot$CELL_NODEF_EXPANSION_SHARE %>% summary()

# This makes the bivariate bins
cells_toplot = 
  cells_toplot %>% 
  bi_class(CELL_NODEF_EXPANSION_SHARE,
            `Cooperative outlet (%)`, 
            dim = 3, 
            style = "quantile", 
                  dig_lab = 9)
...
breaks = 
  cells_toplot %>% 
  bi_class_breaks(CELL_NODEF_EXPANSION_SHARE,
                  `Cooperative outlet (%)`, 
                  dim = 3, 
                  style = "quantile", 
                  dig_lab = 4, 
...
# Calculate the total LU in the aggregate CSS
cells = 
  cells %>% 
  mutate(CELL_COCOA_CSS_HA           = CELL_SHARE_COOP*CELL_COCOA_HA ,
         ...
         CELL_NODEF_EXPANSION_CSS_HA = CELL_SHARE_COOP*CELL_NODEF_EXPANSION_HA)
...
(denseforest_aggr_pcss = cells$CELL_DENSEFOREST_CSS_HA %>% sum())
...
linked_coops = 
  ...
  mutate(
    ...
    COOP_PROP_OF_DENSEFOREST_HA = 100 * COOP_DENSEFOREST_HA / denseforest_aggr_pcss, 

    COOP_PROP_OF_ALL_CD_HA = 100 * COOP_ALL_CD_HA / all_cd_aggr_pcss, 
...
linked_coops$COOP_PROP_OF_DENSEFOREST_HA %>% summary()
...
quantile(linked_coops$COOP_PROP_OF_DENSEFOREST_HA, seq(0,1,0.1))
