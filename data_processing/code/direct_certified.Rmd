...
outsum22_adapted <- read_delim(file = here("temp_data/SEIPCS_adapted_to_IC2B_V2/SEI_PCS_COTE_DIVOIRE_COCOA_2022.csv"), 
                       delim = ",",
...
# for coopbsy it's already done in private_IC2B
...
# Sourcing breakdown to EU (for Nitidae)
```{r}
# output summarized **AND POST TRASE FORMATTING**
outsum20 <- 
  s3read_using(
    object = "cote_divoire/cocoa/sei_pcs/v1.1.0/SEI_PCS_COTE_DIVOIRE_COCOA_2020.csv",
    FUN = read_delim,
    delim = ";",
    bucket = "trase-storage",
    col_types = cols(HS6 = col_character(),
...
outsum21 <- 
  s3read_using(
    object = "cote_divoire/cocoa/sei_pcs/v1.1.0/SEI_PCS_COTE_DIVOIRE_COCOA_2021.csv",
    FUN = read_delim,
    delim = ";",
    bucket = "trase-storage",
    col_types = cols(HS6 = col_character(),
...
outsum22 <- 
  s3read_using(
    object = "cote_divoire/cocoa/sei_pcs/v1.1.0/SEI_PCS_COTE_DIVOIRE_COCOA_2022.csv",
    FUN = read_delim,
    delim = ";",
    bucket = "trase-storage",
    col_types = cols(HS6 = col_character(),
...
# 2019 results, from v1.0.0 model, need some more formatting to match exactly results from v1.1.0 model
outsum19 <- 
  s3read_using(
    object = "cote_divoire/cocoa/sei_pcs/v1.0.0/2021-11-24-SEI-PCS_CI_COCOA_2019_V1.0.0.csv",
    FUN = read_delim,
    delim = ";",
    bucket = "trase-storage",
    col_types = cols(HS6 = col_character(),
  ...
  rename(EXPORTER_GROUP_CLEAN = EXPORTER_CLEAN, 
         EXPORTER_CLEAN = EXPORTER,
         CIF_USD = FOB, 
         LVL4_TRASE_ID_PROD = LVL4_GEOCODE_PROD, 
         LVL4_TRASE_ID_LH = LVL4_GEOCODE_LH) %>% 
  mutate(EXPORTER = NA_character_) %>% 
  select(names(outsum20))
...
outsum_panel <- 
  outsum_panel %>% 
  mutate(IS_INDIRECT = (LVL4_NAME_PROD == "INDIRECT SOURCING"),
         IS_UNKNOWN  = (LVL4_NAME_PROD == "UNKNOWN"),
         IS_DIRECT = !IS_INDIRECT & !IS_UNKNOWN, 
         IS_UNTRACED = !IS_DIRECT,
         IS_DEST_EU = fn_is_in_eu(COUNTRY_OF_DESTINATION), 
...
eu_flow_sum_panel <- 
  ...
  select(YEAR, VOLUME_BEAN_EQUIVALENT_THOUSAND_TONNES, CIF_MILLION_USD, 
         starts_with("PCT_"), 
         starts_with("VOL_"), 
         everything())


write.xlsx(eu_flow_sum_panel,
              file = "C:/Users/guye/OneDrive - UCL/SUSTAINCOCOA/temp_data/content_Trase_release_2024/eu_annual_sourcing_cocoa_civ.xlsx")
