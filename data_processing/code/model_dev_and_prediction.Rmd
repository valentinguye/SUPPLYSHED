...
colnames(summary_data) = c(" ", "JRC Data", "SC Data", "KIT Data", "Cargill Data")
...
table <- summary_data %>%
  ...
  row_spec(7, extra_css = "padding-left: 20px;") %>%
...
map_links = 
  ggplot() +
    ...
    geom_sf(data = coops_toplot, aes(col = Source, 
...
```{r}
# MAIN MODEL NOW 
# Tourne en 16h, mais je pense que ça tournait pas pendant le gros de la nuit, 
# donc plutôt une grosse heure? 
...
model1_rfe <- 
  rfe(form = stg1_formula_allfeatures, 
        ...
        #                  ),
        sizes = c(5, 10, 20, 30, 40, 50, 60, 70), # tell caret how many different feature subset sizes to try. DOes not need to include the total, which is tried out automatically. 
...
saveRDS(model1_rfe, 
        file = here("temp_data", "model", "caret", "stage_1", "allfeatures_ranger_LOOCV_15tuneL_sizeseveryten_repeatedcv5x5"))
...
stg2_allfeatures = 
  ...
  select(
    ...
    starts_with("COOP_STATUS_"), 
    COOP_BS_10KM_TRI, COOP_BS_10KM_SETTLEMENT_HA, 
...
```{r}
# Read caret model 
model1_rfe = readRDS(here("temp_data", "model", "caret", "stage_1", "allfeatures_ranger_100boot632_15tuneL_sizeseveryten_repeatedcv5x5"))
...
```{r}
# Make RSE manually 
mean_stg1_outcome = stg1_development$CELL_PROP_VOLUME_COOPS %>% mean()

stg1_development = 
  stg1_development %>% 
  mutate(NAIVE_MODEL_SQUARED_ERROR = (CELL_PROP_VOLUME_COOPS - mean_stg1_outcome)^2)

stg1_mean_squared_naive_error = stg1_development$NAIVE_MODEL_SQUARED_ERROR %>% mean()

model1_rmse = model1_rfe$fit$results %>% filter(mtry == 26, splitrule == "extratrees") %>% pull(RMSE) 

model1_mse = model1_rmse^2

model1_rse = model1_mse / stg1_mean_squared_naive_error
...
model2_perf = 
model2_caret_xgb$results %>% 
  filter(
  nrounds == 150 & max_depth == 3 & eta == 0.4 & gamma == 0 & colsample_bytree == 0.6 & min_child_weight == 1 & subsample == 0.75
)
...
# Predict link existence only for cells in the AOA   
links = 
  links %>%
...
#           across(.cols = c(COOP_BS_10KM_TRI, COOP_BS_10KM_SETTLEMENT_HA), .fns = ~mean(.x, na.rm = TRUE)),
#           LINK_TRAVEL_MINUTES  = mean(LINK_TRAVEL_MINUTES, na.rm = TRUE),  
#           LINK_TRAVEL_METERS   = mean(LINK_TRAVEL_METERS, na.rm = TRUE),
#           LINK_DISTANCE_METERS = mean(LINK_DISTANCE_METERS, na.rm = TRUE)
#           ) 
...
tic()
cells_topredict$CELL_SHARE_COOP = predict(model1_rfe$fit, newdata = cells_topredict)# add $predicted_outcome if model1 is a familiar model
toc()
...
cells_toplot = 
  ...
  mutate(
   # Work on PERCENTAGE, not share, from here, for display. 
    CELL_SHARE_COOP = 100*CELL_SHARE_COOP_CARET_rfe,
...
links_topredict = 
  links_topredict %>% 
  mutate(
    LINK_PROBA_H10KM = if_else(
      LINK_DISTANCE_METERS <= 10*1e3, 1, 0),
    LINK_PROBA_H30KM = if_else(
      LINK_DISTANCE_METERS <= 30*1e3, 1, 0),
    LINK_PROBA_H60KM = if_else(
      LINK_DISTANCE_METERS <= 60*1e3, 1, 0), 
    LINK_PROBA_HDISTR = if_else(
      CELL_DISTRICT_NAME == COOP_DISTRICT_NAME, 1, 0)
  )

links_topredict$LINK_PROBA_H10KM %>% summary()
links_topredict$LINK_PROBA_H30KM %>% summary()
links_topredict$LINK_PROBA_H60KM %>% summary()
links_topredict$LINK_PROBA_HDISTR %>% summary()
...
saveRDS(links_topredict, 
        here("temp_data", "model", "links_topredict_nopostprocessing.Rdata"))

links_topredict = readRDS(here("temp_data", "model", "links_topredict_nopostprocessing.Rdata"))
```
...
It is not necessary to work on unconditional probabilities.
It is strictly equivalent to uncondition before or after normalising, since unconditioning is multiplying by a cell-level quantity. 
Hence, we join the cell's share of coop outlet, but we don't factor it yet. 

```{r}
links_topredict = 
  links_topredict %>% 
  left_join(
    cells_topredict %>% select(CELL_SHARE_COOP, CELL_ID), 
    by = "CELL_ID"
  ) 
...
#   links_topredict %>% 
#   mutate(
#     LINK_UNCOND_PROBA = CELL_SHARE_COOP * LINK_PROBA, # include this just for exploration. 
#     LINK_UNCOND_PROBA_THRESH = CELL_SHARE_COOP * LINK_PROBA_THRESH
#   )
...
links_topredict = 
  ...
  mutate(
    ...
    CELL_NORMSUM_H10KM  = sum(LINK_PROBA_H10KM), 
    CELL_NORMSUM_H30KM  = sum(LINK_PROBA_H30KM), 
    CELL_NORMSUM_H60KM  = sum(LINK_PROBA_H60KM), 
    CELL_NORMSUM_HDISTR = sum(LINK_PROBA_HDISTR), 
    CELL_N_POTENTIAL_LINKS = length(na.omit(unique(LINK_POTENTIAL_COOP_ID))), 
  ) %>% 
  ungroup() %>% 
  mutate(
    # When denominator is null, i.e. all links in cell have probas = 0, then make the normalised probas ALL AS LIKELY
    # i.e. 1/number of potential links
    # When it's NA, it means we are in a cell outside the AoA, so leave it NA (not necessary anymore, but would need to add this line:       is.na(CELL_NORMSUM_THRESH) ~ NA,
    ...
    LINK_PROBA_H10KM_NORMED = case_when(
      CELL_NORMSUM_H10KM > 0 ~ LINK_PROBA_H10KM / CELL_NORMSUM_H10KM, 
      TRUE ~ 1 / CELL_N_POTENTIAL_LINKS
      ), 
    LINK_PROBA_H30KM_NORMED = case_when(
      CELL_NORMSUM_H30KM > 0 ~ LINK_PROBA_H30KM / CELL_NORMSUM_H30KM, 
      TRUE ~ 1 / CELL_N_POTENTIAL_LINKS
      ), 
    LINK_PROBA_H60KM_NORMED = case_when(
      CELL_NORMSUM_H60KM > 0 ~ LINK_PROBA_H60KM / CELL_NORMSUM_H60KM, 
      TRUE ~ 1 / CELL_N_POTENTIAL_LINKS
      ), 
    LINK_PROBA_HDISTR_NORMED = case_when(
      CELL_NORMSUM_HDISTR > 0 ~ LINK_PROBA_HDISTR / CELL_NORMSUM_HDISTR, 
      TRUE ~ 1 / CELL_N_POTENTIAL_LINKS
      )
  )

# links_topredict %>% 
#   filter(round(CELL_UNCOND_NORMSUM_THRESH,10) != round(CELL_SHARE_COOP * CELL_NORMSUM_THRESH, 10)) %>% nrow()

stopifnot(
links_topredict %>% filter(is.na(LINK_PROBA_THRESH_NORMED)) %>% pull(CELL_IS_IN_AOA) %>% any() == FALSE
)
stopifnot(
links_topredict %>% filter(is.na(LINK_PROBA_H60KM_NORMED)) %>% pull(CELL_IS_IN_AOA) %>% any() == FALSE
)

links_topredict %>% filter(is.na(LINK_PROBA_THRESH_NORMED)) %>% nrow() 

summary(links_topredict$CELL_NORMSUM_THRESH)
# It is not anormal that these sums are higher than 1, that's precisely why we normalise actually. 

# links_topredict %>% filter(CELL_NORMSUM_THRESH > 1) %>% View()


stopifnot(
  links_topredict$LINK_PROBA_THRESH_NORMED %>% sum(na.rm = T) %>% round(5) 
  # + length(unique(pull(filter(links_topredict, CELL_NORMSUM_THRESH == 0), CELL_ID))) # that was for when we (wrongly) set conditional probas to 0 where normsum = 0
  == 
    length(unique(links_topredict$CELL_ID)) |
    
  links_topredict$LINK_PROBA_H60KM_NORMED %>% sum(na.rm = T) %>% round(5) +
    length(unique(links_topredict$CELL_ID)) |

  links_topredict$LINK_PROBA_HDISTR_NORMED %>% sum(na.rm = T) %>% round(5) +
    length(unique(links_topredict$CELL_ID))
)
...
```{r}
# The idea is to export coop-level data sets with sourcing shed shapes, one object for each type, and each time the weighted LU area in all sourcing sheds
