...
selected_links_preds =
  ...
  mutate(LINK_DISTANCE_KM = LINK_DISTANCE_METERS*1e-3,
         ...
         LINK_TRAVEL_MINUTES = LINK_TRAVEL_MINUTES*1e-3, 
         LINK_IS_EXISTING = if_else(LINK_IS_ACTUAL_COOP_class == "Actual_links","Yes","No")
         ) %>% 
  select(#Sample = SPLIT, 
         `Cell-coop link exists` = LINK_IS_EXISTING ,
         `Eucl. distance to a coop (km)` = LINK_DISTANCE_KM, 
         `Road distance to a coop (km)` = LINK_TRAVEL_KM, 
         `Road duration to a coop (min)` = LINK_TRAVEL_MINUTES, 
...
# , Histogram = function(x) hist(list(x))

datasummary_skim(selected_links_preds, fun_numeric = list(Mean = Mean, SD = SD, Min = Min, Median = Median, Max = Max),
...
model1_rfe <- 
  rfe(form = stg1_formula_allfeatures, 
        data = stg1_development,
        trControl = trainControl(method="LOOCV", returnResamp = "all"),
        method = "ranger", 
        importance="permutation", 
        num.trees = 500,
        tuneLength=10,
...
saveRDS(model1_rfe, 
        file = here("temp_data", "model", "caret", "stage_1", "allfeatures_ranger_LOOCV_10tuneL_sizesevery15_cv5"))
...
ggsave(perf_plot_stg1, 
       filename = "perf_allfeatures_ranger_100boot632_15tuneL_sizeseveryten_repeatedcv5x5.png", 
       path = here("outputs"),
       width = 25, 
       height = 20,
       units = "cm")
...
links_topredict$LINK_PROBA_HCONTI %>% summary()
...
links_topredict = 
  ...
  mutate(
    ...
    CELL_NORMSUM_HCONTI = sum(LINK_PROBA_HCONTI), 
    CELL_N_POTENTIAL_LINKS = length(na.omit(unique(LINK_POTENTIAL_COOP_ID))), 
  ) %>% 
  ungroup() %>% 
  mutate(
    # When denominator is null, i.e. all links in cell have probas = 0, then make the normalised probas ALL AS LIKELY (i.e. 1/number of potential links)
    # because the point here is not to reflect unconditional probabilities to sell to a coop, but *conditional* probabilities, i.e. 
    # the probability to sell to coops even very far, very unlikely, if we knew for sure that cocoa is sold to some coops. 
    # In other words, if we knew cocoa is sold to coops and not other buyers, what coops would it be? 
    ...
    LINK_PROBA_HCONTI_NORMED = LINK_PROBA_HCONTI / CELL_NORMSUM_HCONTI,
...
links_topredict$LINK_PROBA_THRESH_NORMED %>% summary() 
links_topredict$LINK_PROBA_HCONTI_NORMED %>% summary() 
links_topredict$LINK_PROBA_H60KM_NORMED %>% summary() 
# links_topredict %>% 
#   filter(round(CELL_UNCOND_NORMSUM_THRESH,10) != round(CELL_SHARE_COOP * CELL_NORMSUM_THRESH, 10)) %>% nrow()

stopifnot(
links_topredict %>% filter(is.na(LINK_PROBA_THRESH_NORMED)) %>% nrow() == 0 
)
stopifnot(
links_topredict %>% filter(is.na(LINK_PROBA_H60KM_NORMED)) %>% nrow() == 0 
)
...
# OUTPUT DESCRIPTIVES 
Do here (for now) the des stats that need objects not available before the Model development part of this script.  
