...
```{r}
# Read caret model 
model1_rfe = readRDS(here("temp_data", "model", "caret", "stage_1", "allfeatures_ranger_100boot632_5tuneL_sizes5-10-50-90_repeatedcv5x5"))
...
set.seed(8888)
all_or_used_features = model1_rfe$optVariables # model1@required_features #stg1_allfeatures #
...
aoa1 = 
  aoa(newdata = cells %>% select(all_of(all_or_used_features)),
      model = model1_rfe$fit$finalModel)
...
model1_caret_cv = 
  train(
    x = stg1_development %>% select(all_of(model1@required_features)), # %>% select(stg1_allfeatures),# 
    y = stg1_development$CELL_PROP_VOLUME_COOPS, 
    method="rf", 
    importance=TRUE, 
    tuneLength=1,
...
cells_topredict$CELL_SHARE_COOP_CARET = predict(model1_caret_cv, newdata = cells_topredict)

model1_caret_boot632 = readRDS(here("temp_data", "model", "caret", "stage_1", "allfeatures_rf_100boot632_5tuneL"))

cells_topredict$CELL_SHARE_COOP_CARET_finalModel = predict(model1_caret_boot632$finalModel, newdata = cells_topredict %>% select(stg1_allfeatures))
...
cells_topredict$CELL_SHARE_COOP_CARET_rfe = predict(model1_rfe$fit, newdata = cells_topredict)
# EXACTEMENT LES MM PREDICTIONS AVEC FINALMODEL OU L'OBJET BOOTSTRAP
cells_topredict$CELL_SHARE_COOP_CARET %>% summary()
cells_topredict$CELL_SHARE_COOP_CARET_finalModel %>% summary()
...
cor.test(cells_topredict$CELL_SHARE_COOP_CARET,
         cells_topredict$CELL_SHARE_COOP_CARET_finalModel)
...
```
#### Compare familiar to other packages 
...
# which is the same as the number of links in cells_topredict outside the AoA. 
