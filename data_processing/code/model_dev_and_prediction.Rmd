...
cells = 
  ...
  mutate(CELL_2ND_STAGE_ONLY = CELL_ANY_ACTUAL_COOP_LINK & !CELL_VOLUME_OBSERVED, 
         CELL_1ST_STAGE_ONLY = !CELL_ANY_ACTUAL_COOP_LINK & CELL_VOLUME_OBSERVED, 
         CELL_BOTH_STAGES = CELL_VOLUME_OBSERVED & CELL_ANY_ACTUAL_COOP_LINK,
...
summon_familiar(
  ...
  skip_evaluation_elements = c("feature_expressions", "feature_similarity", 
                               "fs_vimp",  "hyperparameters", "ice_data", 
                                "permutation_vimp", "model_vimp", "univariate_analysis"), #
...
# ---------------------------------------------
model1_date = "20250114231822" # "20241204190335"
# ---------------------------------------------
...
cells = 
  cells %>% 
  left_join(cells_topredict %>% select(CELL_ID, 
                                       CELL_SHARE_COOP, 
                                       CELL_SHARE_COOP_DRAWLIST, 
                                       CELL_SHARE_COOP_GTRMSE), 
            by = "CELL_ID")
...
cells %>% filter(lengths(CELL_SHARE_COOP_DRAWLIST) == 0) %>% select(CELL_SHARE_COOP_DRAWLIST, everything()) %>% View()
...
# The remaining NAs should be the cells outside of the AoA
...
links = 
  links %>% 
  left_join(
    cells %>% select(CELL_SHARE_COOP, CELL_ID), # CELL_SHARE_COOP, not even sure it is necessary to join this. 
    by = "CELL_ID"
  ) 
