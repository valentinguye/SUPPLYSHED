...
Metric = c("Source", 
  ...
  JRC = c(
          ...
          length(unique(jrcdata$BUYER_GENERAL_ID)),
          length(unique(pull(filter(jrcdata, !is.na(COOP_BS_ID)), BUYER_GENERAL_ID))),
          length(unique(pull(filter(jrcdata, !BUYER_IS_COOP), BUYER_GENERAL_ID)))
          ),
  SC = c(
          ...
          length(unique(scdata$BUYER_GENERAL_ID)), 
          length(unique(pull(filter(scdata, !is.na(COOP_BS_ID)), BUYER_GENERAL_ID))),
          "Unknown"
          ),
  CARGILL = c(
          ...
          length(unique(cargilldata$BUYER_GENERAL_ID)), 
          length(unique(pull(filter(cargilldata, !is.na(COOP_BS_ID)), BUYER_GENERAL_ID))),
          length(unique(pull(filter(cargilldata, !BUYER_IS_COOP), BUYER_GENERAL_ID)))
          )
) 
...
ggplot() +
    ...
    geom_sf(data = consol_coop_sf, aes(col = Source), size = 1) +
    
    scale_colour_manual(
        values = c("#8B0000", "red", "darkgreen")) +
  
  geom_sf(data = unlinked_coopbs_sf, size = 0.1,
            col = "black") +
...
ggplot(toplot) +
  ...
  geom_sf(data = consol_coop_sf, size = 1,
          col = "darkgreen") +
  geom_sf(data = unlinked_coopbs_sf, size = 0.1,
          col = "darkgreen") +
  # scale_size_binned(
