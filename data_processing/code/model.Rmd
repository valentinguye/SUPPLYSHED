...
summon_familiar(
  ...
  experimental_design = "fs + mb", # "bt(fs + mb, 21)", # # "cv(bt(fs,100) + mb, 3)",
  ...
  estimation_type = "point", #list("auc_data"="bci", "model_performance"="point"), # "bootstrap_confidence_interval", # the default. Requires several point estimates. 
...
perf_metrics = read.csv(here(stage1_results, "performance", "performance_metric.csv"))
...
# These are not yet aggregated, they are bootstrap-specific lists of ranked features used in model development.  
...
devdat_prediction = read.csv(here(stage1_results, "prediction", "prediction.csv"))
