...
links = 
  ...
  # Remove the few cells that still fall in no district 
  filter(!is.na(CELL_DISTRICT_NAME)) %>% 
  # Remove the cells where there is little cocoa (less than 1%) and lots of land where cocoa cannot expand (more than 90%).
  filter(!(CELL_COCOA_HA / cell_area_ha < 0.01 & CELL_IMPOSSIBLE_HA / cell_area_ha > 0.9))
...
ggplot(toplot) +
    geom_sf(aes(col = `Cell information content`), size = 0.5) + #, shape = 15
