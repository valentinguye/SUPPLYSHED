...
dir.create(here("temp_data", "caret", "stage_1"), recursive = TRUE)
dir.create(here("temp_data", "caret", "stage_2"), recursive = TRUE)

# These are were summon_familiar will write its outputs
dir.create(here("temp_data", "familiar", "stage_1"), recursive = TRUE)
dir.create(here("temp_data", "familiar", "stage_2"), recursive = TRUE)
...
dir.create(here("temp_data", "predictions", "stage_2"), recursive = TRUE)
...
saveRDS(model1_caret, 
        file = here("temp_data", "caret", "stage_1", "allfeatures_rf_400boot632_5tuneL"))
...
saveRDS(model1_caret_xgb, 
        file = here("temp_data", "caret", "stage_1", "allfeatures_xgbtree_4boot632_5tuneL"))
```
...
stage1_expdes_repo = here("temp_data", "familiar", "stage_1", gsub(",","_",exp_design))
...
# Repository for familiar 1st stage 
# summon_familiar writes all it does in a repo. It returns nothing here.  
stage2_expdes_repo = here("temp_data", "familiar", "stage_2", gsub(",","_",exp_design))
...
stopifnot(sum(is.na(cells$CELL_SHARE_COOP)) == sum(aoa1$AOA==0))
```
...
tic()
links_topredict$LINK_PROBA = predict(model2_caret_xgb, newdata = links_topredict[, stg2_allfeatures], type = "prob")$Actual_links
toc()
...
links = 
  links %>% 
  mutate(LINK_PROBA_THRESH = if_else(LINK_PROBA < thresh, 0, LINK_PROBA))
...
links = 
  ...
  mutate(
    ...
    CELL_NORMSUM = sum(LINK_PROBA), 
    CELL_UNCOND_NORMSUM = sum(LINK_UNCOND_PROBA),
  ...
  mutate(
    # Work non-thresholded proba for now, but we will surely not keep them. 
    LINK_PROBA_NORMED = LINK_PROBA / CELL_NORMSUM, 
    LINK_UNCOND_PROBA_NORMED = LINK_UNCOND_PROBA / CELL_UNCOND_NORMSUM, 
    
    LINK_PROBA_THRESH_NORMED = LINK_PROBA_THRESH / CELL_NORMSUM_THRESH, 
    LINK_UNCOND_PROBA_THRESH_NORMED = LINK_UNCOND_PROBA_THRESH / CELL_UNCOND_NORMSUM_THRESH,
  )

stopifnot(
links %>% filter(is.na(LINK_PROBA_THRESH_NORMED)) %>% filter(CELL_IS_IN_AOA) %>% nrow() == 0
)
...
stopifnot(
  links$LINK_PROBA_THRESH_NORMED %>% sum(na.rm = T) %>% round(5) == length(unique(links_topredict$CELL_ID)) |
  links$LINK_UNCOND_PROBA_THRESH_NORMED %>% sum(na.rm = T) %>% round(5) == length(unique(links_topredict$CELL_ID))
)
...
## Sharp polygons
Sharp means non-overlapping. 
This is an important step, where we define, for every cell, to which coop it most likely supplies. 
We do two versions of this, either focusing on actual or on potential sourcing sheds. 
```{r}
# Find the max conditional, normalised proba in every cell,
# first, if they are all null
links = 
  links %>% 
  group_by(CELL_ID) %>% 
  mutate(
    LINK_PROBA_THRESH_NORMED_CELLMAX = max(LINK_PROBA_THRESH_NORMED), 
    CELL_N_MAX_PROBA = sum(LINK_PROBA_THRESH_NORMED_CELLMAX)
  ) %>% 
  ungroup() %>% 
  mutate(LINK_IS_MOST_LIKELY_IN_CELL = case_when(
    LINK_PROBA_THRESH_NORMED_CELLMAX == 0 | 
    LINK_PROBA_THRESH_NORMED_CELLMAX > LINK_PROBA_THRESH_NORMED ~ FALSE, 
    TRUE ~ TRUE
  )
) 

# How to handle equi-probabilities? 
links %>%   
  filter(CELL_N_MAX_PROBA > 1) %>%   
  View()
...
links = 
  links %>% 
  mutate(
    CELL_IN_SOURCING_SHED = CELL_SHARE_COOP_SIGNI > 0, 
     # This is where no cocoa is estimated to be sold to coops, but independent of that, some coops are still predicted to source from there.  
     CELL_IN_INFLUENCE_SHED = CELL_SHARE_COOP_SIGNI == 0 & CELL_N_MAX_PROBA > 0)
