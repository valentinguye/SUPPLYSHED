...
stg1_allfeatures = 
  ...
  select(
    ...
    CELL_TRI_MM, CELL_N_BS_WITHIN_DIST, CELL_N_COOP_IN_DPT, CELL_MIN_DISTANCE_METERS
  ) %>%
  names()
...
summon_familiar(
  ...
  formula = stg2_formula_allfeatures,
  data = stg2_development %>% select(LINK_ID, LINK_IS_ACTUAL_COOP_class, all_of(stg2_allfeatures)) %>% drop_na(),
...
saveRDS(model1_prediction, 
        here("temp_data", "predictions", "stage_1", ""))
