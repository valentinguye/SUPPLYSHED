...
cells = 
  cells %>%  
  filter(!(CELL_COCOA_HA < 0.01*1600 & CELL_IMPOSSIBLE_HA > 0.8*1600))
...
summon_familiar(
  ...
  hyperparameter = list("xgboost_tree"=
                          list("n_boost" = 2, # this means 10^2 trees
                                "learning_rate" = -1.5,
                                "alpha" = 0, 
                                "lambda" = 1
                                )),
  ...
  skip_evaluation_elements = c("feature_expressions", "feature_similarity", 
                               "fs_vimp",  "hyperparameters", "ice_data", 
                                "model_vimp", "permutation_vimp", "univariate_analysis"), #
...
# Hold-out validation (deprecated) ---------
...
## Predict 1st stage
```{r}
cells_topredict$CELL_PREDICTED_PROP_VOLUME_COOPS = predict(model1, newdata = cells_topredict)$predicted_outcome

# saveRDS(model1_prediction, 
#         here("temp_data", "predictions", "stage_1", paste0(model_date, "_inaoa_prediction")))


cells_topredict$CELL_PREDICTED_PROP_VOLUME_COOPS %>% summary()

cells = 
  cells %>% 
  left_join(cells_topredict %>% select(CELL_ID, CELL_PREDICTED_PROP_VOLUME_COOPS), 
            by = "CELL_ID")

cells =
  cells %>% 
  mutate(CELL_PREDICTED_PROP_VOLUME_COOPS = if_else(CELL_NO_POTENTIAL_LINK, 
                                                    0, 
                                                    CELL_PREDICTED_PROP_VOLUME_COOPS))

cells$CELL_PREDICTED_PROP_VOLUME_COOPS %>% summary()

stopifnot(sum(is.na(cells$CELL_PREDICTED_PROP_VOLUME_COOPS)) == sum(aoa1$AOA==0))
...
## Predict 1st stage
```{r}
links_topredict$LINK_PREDICTED_IS_ACTUAL_COOP_class  = predict(model2, newdata = links_topredict)$predicted_outcome
# Since this is quite long, we save the output
saveRDS(links_topredict,
        here("temp_data", "predictions", "stage_2", paste0(model_date, "_inaoa_prediction")))

links_topredict$LINK_PREDICTED_IS_ACTUAL_COOP_class %>% summary()

links = 
  links %>% 
  left_join(links_topredict %>% select(LINK_ID, LINK_PREDICTED_IS_ACTUAL_COOP_class), 
            by = "LINK_ID")

links =
  links %>% 
  mutate(LINK_PREDICTED_IS_ACTUAL_COOP_class = if_else(CELL_NO_POTENTIAL_LINK, 
                                                    "Virtual links", 
                                                    LINK_PREDICTED_IS_ACTUAL_COOP_class))

links$LINK_PREDICTED_IS_ACTUAL_COOP_class %>% summary()
