  ...
  cells %>% 
    mutate(
      `Cell type` = case_when(
        ...
        CELL_ONLY_VIRTUAL_LINK ~ "No actual link data",
        CELL_ACTUAL_LINK & !CELL_VOLUME_OBSERVED ~ "Data on actual link existence only",
        CELL_ACTUAL_LINK & CELL_VOLUME_OBSERVED ~ "Data on actual link existence and size",
      )) %>% 
  st_as_sf(coords = c(""))
toplot$`Cell type` %>% table()    
...
### Remove No-potential-links cells here 
```{r}
